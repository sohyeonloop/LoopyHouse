...
@WebServlet("/comm/inquery.do")
public class InqBoardController extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public InqBoardController() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//게시판 글 조회
				...
				//전체 글 갯수 조회하기
				int countList = service.countIBoard();
//				System.out.println("전체글 갯수 : " + countList);
				...
				map.put("start", start);
				map.put("end", end);
				List<InqBoardVO> boardList = service.inqBoardList(map);
				...
				request.getRequestDispatcher("/inquery").forward(request, response);
	}

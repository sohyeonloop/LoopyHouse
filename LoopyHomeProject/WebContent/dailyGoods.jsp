...
if (request.getParameter("check") == null || request.getParameter("check").equals("")) {
	listP = (ArrayList<ProductVO>) prodDao.selectCateProd("DS001");
} else if (request.getParameter("check").equals("total")) {
	listP = (ArrayList<ProductVO>) prodDao.selectCateProd("DS001");
} else if (request.getParameter("check").equals("sale")) {
	listP = (ArrayList<ProductVO>) prodDao.selectSaleProd("DS001");
} else if (request.getParameter("check").equals("wish")) {
	listP = (ArrayList<ProductVO>) prodDao.selectWishProd("DS001");
}
		...
		<div class="container">
			...
			<ul class="dropdown-menu">
				<li class="total-prod"><a class="dropdown-item "
					href="${pageContext.request.contextPath}/dailyGoods.jsp?check=total">전체</a></li>
				<li><hr class="dropdown-divider"></li>
				<li class="sale-prod"><a class="dropdown-item"
					href="${pageContext.request.contextPath}/dailyGoods.jsp?check=sale">판매순</a></li>
				<li><hr class="dropdown-divider"></li>
				<li class="sale-prod"><a class="dropdown-item"
					href="${pageContext.request.contextPath}/dailyGoods.jsp?check=wish">위시리스트순</a></li>
			</ul>
		</div>
		...
		<div class="row" align="center">
			...
			<div class="col-md-4">
				...
				<%
					if (wishCheck) {
				%>
				<i class="bi bi-heart-fill wish"></i><%=pv.getProdWish()%></span>
				<%
					} else if (!wishCheck) {
				%>
				<i class="bi bi-heart wish"></i><%=pv.getProdWish()%></span>
				<%
					}
				%>
